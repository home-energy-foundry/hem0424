...
class Ductwork:
    ...
    def __init__(self, internal_diameter, external_diameter, length, k_insulation, thickness_insulation, reflective, MVHR_location):
    ...
    def total_duct_heat_loss(self, outside_temp, supply_duct_temp, extract_duct_temp, intake_duct_temp, exhaust_duct_temp):
        """" Return the heat loss for air inside the duct for the current timestep
        Arguments:
        outside_temp       -- temperature outside the duct, in degrees C
        ...
        if self.__MVHR_location == 'outside':
            supply_heat_loss = self.duct_heat_loss(supply_duct_temp, outside_temp,)
            extract_heat_loss = self.duct_heat_loss(extract_duct_temp, outside_temp,)
            total_duct_heat_loss = supply_heat_loss + extract_heat_loss

        # Inside location
        # This will be a negative heat loss i.e. air inside the duct gains heat, dwelling loses heat
        elif self.__MVHR_location == 'inside':
            intake_heat_loss = self.duct_heat_loss(intake_duct_temp, outside_temp,)
            exhaust_heat_loss = self.duct_heat_loss(exhaust_duct_temp, outside_temp,)
            total_duct_heat_loss = intake_heat_loss + exhaust_heat_loss
